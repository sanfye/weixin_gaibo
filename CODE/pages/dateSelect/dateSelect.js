...
Page({
  ...
  createDateListData:function(){
    ...
    */
    ...
    for(var i=0;i<this.data.maxMonth;i++){
      var momentDate = Moment(now).sub(this.data.maxMonth-(i+1),'month').date;
      ...
      for(var j = -week+1;j<=totalDay;j++){
          ...
          if(j==DATE_DAY && year == DATE_YEAR && month == DATE_MONTH)
              //当天之前的日期不可用
              clazz = 'current '+clazz;
          else if(year == DATE_YEAR && ((j > DATE_DAY && month == DATE_MONTH) || (month > DATE_MONTH)))
  ...
  onPressDate:function(e){
    ...
    //当前选择的日期为同一个月并大于今天，或者点击了空白处（即day<0）,或者月份大于当前月，不执行
    if((day>DATE_DAY && month == DATE_MONTH) || day<=0 || month > DATE_MONTH) return;
