...
Page({
  ...
  queryOrderInfo: function () {
    ...
    var selectmachineCode = this.data.array[this.data.index];
    var param = 'startTime=' + startTime + '&endTime=' + endTime + '&userName=' + userName + '&password=' + password + '&selectmachineCode=' + selectmachineCode;

    // 页面显示http://127.0.0.1:8180/queryOrderInfo/orderInfo?
    api.get('https://www.gablecafe.cn/gaibo-query-server/queryOrderInfo/orderInfo?' + param)
      .then(res => {
        ...
        if (res == null || res.data == null) {
          ...
          var machineList = res.data.machines;
          var jsonArray = [];
          var totalPrices = 0;
